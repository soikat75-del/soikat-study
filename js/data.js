/* ==========================================
   SOIKAT BCS HUB
   Data
========================================== */

const quotes = [

    // Bangla

    "আজকের পরিশ্রমই আগামী দিনের সাফল্য।",

    "শৃঙ্খলা প্রতিভার চেয়েও শক্তিশালী।",

    "প্রতিদিন একটু এগোলেই একদিন অনেক দূর পৌঁছে যাবে।",

    "স্বপ্ন পূরণ হয় ধারাবাহিক পরিশ্রমে।",

    "যে পড়ে, সেই এগিয়ে যায়।",

    "হাল ছেড়ো না। প্রতিটি অধ্যায় তোমাকে লক্ষ্য্যের আরও কাছে নিয়ে যাচ্ছে।",

    "নিজের সঙ্গে প্রতিযোগিতা করো।",

    "সাফল্য কখনো শর্টকাটে আসে না।",

    "আজ যা শিখবে, কাল সেটাই তোমার শক্তি হবে।",

    "প্রতিদিন ১% উন্নতি, এক বছরে বিশাল পরিবর্তন।",

    // English

    "Discipline beats motivation.",

    "Consistency creates excellence.",

    "Small progress is still progress.",

    "Stay focused. Stay patient.",

    "Success is earned every day.",

    "Do today's work today.",

    "Never skip the difficult chapter.",

    "Your future is built today.",

    "Focus on progress, not perfection.",

    "One page today. One chapter tomorrow."

];


/* Future Search Database */

const searchDatabase = [

    {
        title:"Subjects",
        description:"Browse all BCS subjects",
        url:"pages/subjects.html"
    },

    {
        title:"Strategy",
        description:"Preparation strategy",
        url:"pages/strategy.html"
    },

    {
        title:"Study Tracker",
        description:"Track daily study",
        url:"pages/tracker.html"
    },

    {
        title:"Planner",
        description:"Daily planner",
        url:"pages/planner.html"
    }

];
